const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:18090';
...
const adminCookie = await login('admin@ev-charging-operations.local', 'admin123');
const managerCookie = await login('manager@ev-charging-operations.local', 'manager123');
const analystCookie = await login('analyst@ev-charging-operations.local', 'analyst123');
